...
public class RepresentationVue extends Stage implements Observer{
	
	protected ProjectionRepresentation rp;

	
	public RepresentationVue(ProjectionRepresentation rp) {

		this.rp.attach(this);
		
		Scene sc = new Scene(SceneContent(),1500,790);
		this.setScene(sc);
		
		update(rp);  // initialize display to current model value
		...
		Stage primaryStage = new Stage();
			...
			openFileItem.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					if (file != null) {
						RepresentationVue.this.file = file.getAbsolutePath();
						chargeFichier();
						affichagePly();
					}
			...
			listPly.setOnMouseClicked(e -> {
				if (!this.file.equals(path + File.separator + listPly.getSelectionModel().getSelectedItem())) {
					...
					RepresentationVue.this.file = path + File.separator + listPly.getSelectionModel().getSelectedItem();
					System.out.println(this.file);
					chargeFichier();
					affichagePly();
				}
