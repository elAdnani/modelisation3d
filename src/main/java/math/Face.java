...
public class Face {
	...
	private static final Vecteur SOURCELUMINEUSE = FigureFabrique.getInstance().vecteur(0, 0, 1);;
	...
	public void divisionTriangulaire() {
		...
		Point milieuAB = fabriquePoint.point((a.getX() + b.getX()) / 2, (a.getY() + b.getY()) / 2, (a.getZ() + b.getZ()) / 2);
		Point milieuAC =fabriquePoint.point((a.getX() + c.getX()) / 2, (a.getY() + c.getY()) / 2, (a.getZ() + c.getZ()) / 2);
		Point milieuBC = fabriquePoint.point((b.getX() + c.getX()) / 2, (b.getY() + c.getY()) / 2, (b.getZ() + c.getZ()) / 2);
	...
	public double degreDeCouleur() {
		FigureFabrique fabriqueVecteur = FigureFabrique.getInstance();
		double degreDeCouleur = -1;
		Vecteur normalUnitaire = fabriqueVecteur.vecteur();
		if (this.points.size() >= 3) {
			normalUnitaire = VecteurOutil.normalUnitaire(this.points.get(0), this.points.get(1), this.points.get(2));
